    ...
    setCart(prevCart =>
      prevCart.map(item =>
        item.service._id === serviceId
          ? { ...item, quantity: Math.round(quantity * 10) / 10 } // Round to 1 decimal place
          : item
      )
    );
  }, [removeFromCart]);
    ...
    <div className="min-h-screen bg-gray-50 flex">
      ...
      <div className="w-96 bg-white border-l shadow-lg flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          {/* Cart Items */}
          {orderLoading ? (
            <div className="flex items-center justify-center h-32 text-mint-green">
            ...
            <div className="space-y-4">
              {cart.map((item, idx) => (
                <Card key={item.service._id} className="luxury-card p-2 rounded-xl shadow-md flex flex-col items-center gap-1">
                  <div className="font-semibold text-sm text-gray-900 text-center truncate w-full">
                    {item.service.name}
                  </div>
                  <div className="text-mint-green font-bold text-base">
                    Ksh {item.service.price}
                  </div>
                  <div className="text-xs text-gray-500 mb-1">
                    {item.service.category.includes('cleaning') ? 'per sqm' : 'per kg'}
                  </div>
                  <div className="flex items-center gap-2 justify-center w-full">
                    <label htmlFor={`cart-qty-${item.service._id}`} className="text-xs text-gray-600">Qty:</label>
                    <Input
                      id={`cart-qty-${item.service._id}`}
                      type="number"
                      min={0.1}
                      step={0.1}
                      value={item.quantity}
                      onChange={e => {
                        let qty = parseFloat(e.target.value) || 0.1;
                        if (qty < 0.1) qty = 0.1;
