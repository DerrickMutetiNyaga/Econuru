          ...
          <div key={card.title} className={statCardClass}>
            <div>
              ...
              <div className="flex items-center mt-2">
                ...
                <span className={`text-sm font-medium ${card.changeType === "positive" ? "text-green-600" : "text-red-600"}`}>{card.change}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mt-4">
        {monthlyCards.map((card, index) => (
          <div key={card.title} className={statCardClass}>
            <div>
              <p className="text-sm font-medium text-text-light">{card.title}</p>
              <p className={`text-2xl font-bold ${card.title === 'Month Expenses' ? 'text-red-600' : card.title === 'Month Unpaid Laundry' ? 'text-yellow-600' : card.title === 'Month Profit/Loss' && monthly.profit < 0 ? 'text-red-600' : 'text-green-700'}`}>{card.value}</p>
              <div className="flex items-center mt-2">
                {card.changeType === "positive" ? (
                  <ArrowUp className="w-4 h-4 text-green-600 mr-1" />
                ) : (
              ...
              {card.title === 'Month Profit/Loss' && (
                <p className="text-xs text-gray-500 mt-1">Daily: <span className={card.dailyType === 'positive' ? 'text-green-700' : 'text-red-600'}>{card.daily}</span></p>
              )}
            </div>
